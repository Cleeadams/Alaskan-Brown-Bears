...
tab <- read.delim("USGS Russell River edit.txt")
...
# Plot data
slr <- lm(newsst~exp.date)
plot(exp.date,newsst)
abline(slr)
exponential.model <- lm(log(newsst)~exp.date)
summary(exponential.model)
timevalues <- seq(1990,2022,.1)

sst.exponential2 <- exp(predict(exponential.model,list(exp.date=timevalues)))

lines(timevalues, sst.exponential2,lwd=2, col = "red")
